...
| **scaffold** | `cargo loco generate scaffold <name> [field:type ...] [--without-tz]` | `with-db` | Full CRUD: entity, migration, controller, routes, tests — plus typed React hooks/pages when the app has a `frontend/`. Adaptive, no kind flag. `lib.rs:102`, `cli.rs:268` |
| **controller** | `cargo loco generate controller <name> [action ...]` | none | Controller (JSON API) + routes + tests only — no model/migration. `lib.rs:117`, `cli.rs:299` |
...
# scaffold (model + controller + tests; adds React hooks/pages if the app has a frontend/)
