...
use roco_gen::{collect_messages, generate, AppInfo, Component};
use rrgen::RRgen;
use std::fs;
...
fn can_generate() {
    ...
    assert_eq!(
        ...
        r"* Migration for `movies` added! You can now apply it with `$ cargo roco db migrate && cargo roco db entities`.
