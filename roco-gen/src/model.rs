...
pub fn generate(
    ...
    if std::env::var("SKIP_MIGRATION").is_err() {
        ...
        let _ = cmd!("cargo", "roco-tool", "db", "migrate",)
            .stderr_to_stdout()
            .dir(cwd.as_path())
            .full_env(&env_map)
            .run()
            .map_err(|err| {
                Error::Message(format!(
                    "failed to run roco db migration. error details: `{err}`",
                ))
            })?;
        let _ = cmd!("cargo", "roco-tool", "db", "entities",)
            .stderr_to_stdout()
            .dir(cwd.as_path())
            .full_env(&env_map)
            .run()
            .map_err(|err| {
                Error::Message(format!(
                    "failed to run roco db entities. error details: `{err}`",
                ))
            })?;
    }
