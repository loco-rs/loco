    ...
    pub fn col_type(&self) -> String {
        ...
        match &self.kind {
            ...
            ColumnKind::Scalar(scalar) => match scalar {
                ...
                // Emit a signed `SmallInteger` (i16) for both, not sea-orm's leaky
                ...
                ScalarType::SmallInt | ScalarType::SmallUnsigned => flag.suffixed("SmallInteger"),
                // `int` is 64-bit (see `scalar_from_base_name`): portable across
                // SQLite (no 32-bit integer affinity) and Postgres.
                ScalarType::Int | ScalarType::BigInt => flag.suffixed("BigInteger"),
...
const fn array_col_type_name(scalar: &ScalarType) -> &'static str {
    match scalar {
...
const fn scalar_ts_override(scalar: &ScalarType) -> Option<&'static str> {
    match scalar {
        ...
        | ScalarType::Time
        | ScalarType::DateTime
        | ScalarType::DateTimeTz => Some("string"),
