...
// when running locally set ROCO_DEV_MODE_PATH=<to local roco path>
#[rstest::rstest]
// lightweight service
...
fn test_combination(db: DBOption, asset: AssetsOption, test_generator: bool) {
    ...
    let settings =
        settings::Settings::from_wizard("test-roco-template", &wizard_selection, OS::default());
...
impl Tester {
    ...
    fn run_generate(&self, command: &Vec<&str>) {
        let base_command = vec!["roco", "generate"];
    ...
    fn run_generate_migration(&self, command: &Vec<&str>) {
        let base_command = vec!["roco", "generate", "migration"];
