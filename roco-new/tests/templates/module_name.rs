use roco::{settings, wizard::DBOption};
use rstest::rstest;
...
pub fn run_generator() -> TestGenerator {
    let settings = settings::Settings {
        package_name: "roco-app-test".to_string(),
        module_name: "roco_app_test".to_string(),
        ..Default::default()
    };
...
fn test_cargo_toml() {
    ...
    assertion::toml::assert_path_value_eq_string(&content, &["package", "name"], "roco-app-test");
    ...
    assertion::toml::assert_path_value_eq_string(bin, &["name"], "roco_app_test-cli");
}
...
fn test_use_name(#[values("src/bin/main.rs", "tests/requests/home.rs")] file: &str) {
    ...
    assertion::string::assert_line_regex(&content, "(?m)^use roco_app_test::");
}
