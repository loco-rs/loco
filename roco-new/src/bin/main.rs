    ...
    let res = match cli.command {
        Commands::New {
            ...
            if to.exists() {
                ...
                if let Ok(path) = env::var("ROCO_DEV_MODE_PATH") {
                    println!("⚠️ NOTICE: working in dev mode, pointing to local Roco on '{path}'");
                }
                ...
                let res = match Generator::new(Arc::new(executor), settings).run() {
                    Ok(()) => {
                        ...
                        tracing::debug!("roco template app generated successfully",);
